...
EKSの場合ですと、デフォルトでは「Amazon VPC CNI plugin for Kubernetes」を使用していますが、2025/08時点では「Amazon VPC CNI plugin for Kubernetes」はNetworkPolicyに対応しています。
他にNetworkPolicyに対応したCNIプラグインとしてはCalicoなどがあります。  

## Amazon VPC CNI plugin for KubernetesにNetworkPolicyを適用させる  

Amazon VPC CNIにNetworkPolicyを適用させます。[公式の手順][3]を参考にしてください。  
ステップ2のみを設定すれば問題ないですが、出来なかった場合は他のステップも設定する、または次節の「Calicoのデプロイ」を実行してください。
...
[3]:https://docs.aws.amazon.com/ja_jp/eks/latest/userguide/cni-network-policy-configure.html
