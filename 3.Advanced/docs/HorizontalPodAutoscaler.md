...
また、Pod（コンテナ）にresources.requestsを指定していることも条件になります。HPAは目標の負荷状態を宣言します。
...
2. 作成したオブジェクトを確認してください。Podが1つであることを確認してください。
...
4. 上記作成したPodに以下のような追加コマンドを発行してください。(宛先は作成したService名にする。最後の/を忘れずに。-nと-cの値はお好みで)

   ```bash
   ab -n 1000000 -c 100 http://httpd-svc/
   ```

5. 別ターミナルを開き、「kubectl top pod」と「kubectl get pod」をwatch等で監視し、以下を確認してください。
   - しばらくするとPodの数が増えること
   - Podが5つまで自動拡張されること
