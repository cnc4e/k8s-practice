...
DaemonSetは常にすべてのワーカーノードで同じPodを起動するPodコントローラーです。クラスタ管理系機能（ログやメトリクスの収集）によく使われます。

1. クラスタの参加ノードを確認し、ワーカーノードが2台以上の状態にしてください。

   【回答例】

2. 以下を満たすマニフェストを作成しデプロイしてください。なお、DaemonSetについては[公式ドキュメント][1]を参考にしてください。
...
4. Podリソースのオブジェクト一覧を起動ノード名とともに表示し、すべてのワーカーノードで`daemon`Podが起動していることを確認してください。
