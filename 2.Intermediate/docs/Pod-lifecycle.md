...
Podを手動で停止(kubectl delete)すると、SIGTERMが送られ30秒の猶予期間後にSIGKILLでプロセス強制終了となります。
SIGTERMやSIGKILLで終了したくないコンテナにはpreStopでプロセスの終了コマンドを実行しましょう。
...
1. Deployment:`nginx`のマニフェストを以下のように修正して再デプロイしてください。
...
1. Pod:`lifecycle`を何度か手動で削除しセルフ・ヒーリングさせてください。（Deploymentは消さない）

1. Pod:`curl`から`lifecycle-svc`に対しcurlコマンドを実行します。Podを作成した時刻が`一行のみ`出力されることを確認してください。
