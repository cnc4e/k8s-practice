...
1. デプロイしたPodの詳細を表示し、各コンテナにresourcesが設定されていることを確認してください。またmemoryの`requestsとlimitsが同じ値`で設定されていることを確認してください。
...
1. STATUS:PendingのPodの詳細を表示しeventを確認してください。以下のようなメッセージが出力されていることを確認してください。（これはワーカー3台の環境で要求した量のCPUを確保できるノードが見つからなかった場合のメッセージです。）
