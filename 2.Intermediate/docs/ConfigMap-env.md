...
また、ConfigMapは複数のPodに同じConfigMapを紐づけることができるため、複数Podで使用する環境変数の集中管理も可能です。
...
2. 各Deploymentで展開したPodを削除し、セルフ・ヒーリングさせてください。

3. 各Deploymentで展開したPodのログを表示してください。表示が変わることを確認してください。
