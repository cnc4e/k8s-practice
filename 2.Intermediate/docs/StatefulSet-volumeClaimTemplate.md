...
EBSは単一のEC2インスタンスにしかボリュームをアタッチできません。
...
また、Podがセルフ・ヒーリングで再作成された場合はそのPodに対応したボリュームが引き続き使用されます。
なお、volumeClaimTemplateで作成されたPVCおよびPVはStatefulSetを削除しても`消えません`。（データを残すためにあえてこのような仕様になっていると思われます。）
