...
Secretはdata部分がbase64でエンコードされます。（base64デコードすることで簡単に復元できます。）
ログイン情報など、値を隠したい情報を格納する時に使用します。（ただし繰り返しになりますが、単純にbase64エンコードしたのみなので、base64デコードで簡単に復元できます。）
...
1. 以下のコマンドを実行してください。valueの中身が復元されることを確認してください。
