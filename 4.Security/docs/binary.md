...
正しいバイナリを使用してコンポーネントを動かすことも重要です。巧妙に罠を仕掛けられた偽のバイナリで動かすと知らない内に危険にさらされているかもしれません。K8sのコントロールプレーンコンポーネントにおいても同じです。信頼できる入手元のバイナリと一致しているか確かめる方法を実践します。
...
5. 以下コントロールプレーンコンポーネントのハッシュ値(SHA-256)を取得し、以下に書かれているハッシュ値と同じか確認してください。いくつかのコンポーネントは意図的に違うハッシュ値を書いています。（ヒント：SHA-256のハッシュ値を得るにはsha256sumコマンドを使います。） 

> （2021/09/13 追記）以下のハッシュ値は問題作成時のハッシュ値です。現在はバイナリが変わっておりハッシュ値も全くことなります。そのため、以下のハッシュ値との比較はせず、ハッシュ値を得る方法とそれを比較する方法だけ答えてください。
...
本来なら比較用のハッシュ値は信頼できる提供元にある値と比較します。今回はコントロールプレーンコンポーネントのバイナリで実践しましたが、他のインターネットから入手したものは大体同じことが言えます。それらしいものもちゃんと提供元のハッシュ値と同じものか確認することでより安全と言えます。
